...
export const metadata: Metadata = {
  title: 'Jobify Dev',
  description: 'Page application tracking system for job hunters',
};
```
...
5. **Handle loading and empty states**
   ...
   - If `jobs` is an empty array, return a `h2` element with 'No Page Found...' as its child.
...
function JobsList() {
  ...
  if (jobs.length < 1) return <h2 className='text-xl'>No Page Found...</h2>;
  return (
    <>
      {/*button container  */}
